"""
Decision Contracts for Consensus Technique
...
Note: Examples use SQL for illustration, but these contracts are generic
and can be used for any code type (Python, JavaScript, etc.).
"""
...
class ErrorCategory(str, Enum):
    """Categories of code compilation/execution errors"""

    TYPE_MISMATCH = "type_mismatch"
    SYNTAX_ERROR = "syntax_error"
    SCHEMA_ISSUE = "schema_issue"
    DEPENDENCY_ERROR = "dependency_error"  # Generic replacement for ref/source/macro errors
    PERMISSION_ERROR = "permission_error"
    TIMEOUT_ERROR = "timeout_error"
    TEMPLATE_ERROR = "template_error"  # Generic replacement for jinja_error
...
class FixStrategy(str, Enum):
    ...
    DEPENDENCY_FIX = "dependency_fix"  # Generic replacement for macro/ref/source fixes
...
class LocationDecision(BaseModel):
    ...
    confidence: float = Field(ge=0.0, le=1.0)
    source_hash: str = Field(description="SHA256 of original code to detect mid-pipeline changes")
    provenance: dict = Field(default_factory=lambda: {"timestamp": time.time()})
...
class FixDecision(BaseModel):
    ...
    def validate_no_multiple_statements(self) -> Any:
        """Detect potential code injection (multiple statements in single fix)"""
        # Check for semicolons in middle of code (not at end) - common in SQL/JS/C-like languages
        code = self.new_code.strip().rstrip(";")
        if ";" in code:
            msg = (
                "new_code contains multiple statements (semicolon in middle). "
                "Atomic fixes should be single-statement changes."
            )
            raise ValueError(msg)
        return self

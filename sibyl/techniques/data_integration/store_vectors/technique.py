"""Store vectors technique orchestrator.

This technique manages vector storage and statistics operations.
"""
...
class StoreVectorsTechnique(BaseTechnique):
    """Technique for storing and managing vectors in vector databases."""
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            subtechnique: Subtechnique name (storer or get_stats)
            variant: Variant name (default, provider, custom)
    ...
    def _discover_subtechniques(self) -> None:
        ...
        # Discover storer and get_stats subtechniques
        for subtechnique_name in ["storer", "get_stats"]:
            self._discover_subtechnique_variants(subtechnique_name, base_path)
        ...
        for variant in ["default", "provider", "custom"]:
            ...
            if variant_path.exists():
                try:
                    ...
                    module_path = f"sibyl.techniques.data_integration.store_vectors.subtechniques.{subtechnique_name}.{variant}"
...
def build_technique(config_path: Path | None = None) -> StoreVectorsTechnique:
    """Build and return the store vectors technique instance.

    Args:
        config_path: Optional path to technique config file

    Returns:
        StoreVectorsTechnique instance
    """
    return StoreVectorsTechnique(config_path)

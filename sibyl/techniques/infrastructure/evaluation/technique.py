...
class EvaluationTechnique(BaseTechnique):
    """Response evaluation and quality metrics."""

    def __init__(self, config_path: Path | None = None) -> None:
        self._name = "evaluation"
        self._description = "Response evaluation and quality metrics"
    ...
    def register_subtechnique(
        ...
        logger.debug("Registered evaluation subtechnique %s:%s", subtechnique_name, impl_name)
    ...
    def load_config(self, config_path: Path) -> dict[str, Any]:
        if not config_path.exists():
            logger.warning("Config file not found for evaluation: %s", config_path)
            return {}
        try:
    ...
    def _discover_subtechniques(self) -> None:
        base = "sibyl.techniques.infrastructure.evaluation.subtechniques"
        mapping = {
            "faithfulness": [
                (
                    f"{base}.faithfulness.default.source_attribution",
                    "SourceAttributionImplementation",
                ),
                (f"{base}.faithfulness.default.context_check", "ContextCheckImplementation"),
            ],
            "groundedness": [
                (f"{base}.groundedness.default.fact_check", "FactCheckImplementation"),
                (
                    f"{base}.groundedness.default.hallucination_detect",
                    "HallucinationDetectImplementation",
                ),
            ],
            "relevance": [
                (f"{base}.relevance.default.keyword_match", "KeywordMatchImplementation"),
                (
                    f"{base}.relevance.default.semantic_similarity",
                    "SemanticSimilarityImplementation",
                ),
            ],
            "coherence": [
                (f"{base}.coherence.default.flow_check", "FlowCheckImplementation"),
            ],
            "completeness": [
                (f"{base}.completeness.default.coverage_check", "CoverageCheckImplementation"),
            ],
        }
    ...
    def _load_and_register(self, subtechnique: str, module_path: str, class_name: str) -> None:
        try:
            ...
            logger.warning(
                "Failed to register evaluation implementation %s.%s: %s",
                module_path,
                class_name,
                exc,
            )

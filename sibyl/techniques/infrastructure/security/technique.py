...
class SecurityTechnique(BaseTechnique):
    """Security and compliance controls orchestrator."""

    def __init__(self, config_path: Path | None = None) -> None:
        self._name = "security"
        self._description = "Security and compliance controls"
    ...
    def register_subtechnique(
        ...
        logger.debug("Registered security subtechnique %s:%s", subtechnique_name, impl_name)
    ...
    def load_config(self, config_path: Path) -> dict[str, Any]:
        if not config_path.exists():
            logger.warning("Config file not found for security: %s", config_path)
            return {}
        try:
    ...
    def _discover_subtechniques(self) -> None:
        base = "sibyl.techniques.infrastructure.security.subtechniques"
        mapping = {
            "content_filtering": [
                (f"{base}.content_filtering.default.keyword_filter", "KeywordFilterImplementation"),
                (f"{base}.content_filtering.default.pattern_filter", "PatternFilterImplementation"),
            ],
            "prompt_injection_detection": [
                (
                    f"{base}.prompt_injection_detection.default.pattern_based",
                    "PatternBasedImplementation",
                ),
                (f"{base}.prompt_injection_detection.default.ml_based", "MlBasedImplementation"),
            ],
            "audit_logging": [
                (f"{base}.audit_logging.default.file_log", "FileLogImplementation"),
                (f"{base}.audit_logging.default.db_log", "DbLogImplementation"),
            ],
            "access_control": [
                (f"{base}.access_control.default.rbac", "RbacImplementation"),
                (f"{base}.access_control.default.abac", "AbacImplementation"),
            ],
            "pii_redaction": [
                (f"{base}.pii_redaction.default.regex_redaction", "RegexRedactionImplementation"),
                (f"{base}.pii_redaction.default.spacy", "SpacyImplementation"),
                (f"{base}.pii_redaction.default.presidio", "PresidioImplementation"),
            ],
        }
    ...
    def _load_and_register(self, subtechnique: str, module_path: str, class_name: str) -> None:
        try:
            ...
            logger.warning(
                "Failed to register security implementation %s.%s: %s",
                module_path,
                class_name,
                exc,
            )

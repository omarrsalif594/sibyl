...
class WorkflowOptimizationTechnique(BaseTechnique):
    """Workflow optimization strategies."""

    def __init__(self, config_path: Path | None = None) -> None:
        self._name = "workflow_optimization"
        self._description = "Workflow optimization strategies"
    ...
    def register_subtechnique(
        ...
        logger.debug(
            "Registered workflow_optimization subtechnique %s:%s", subtechnique_name, impl_name
        )
    ...
    def load_config(self, config_path: Path) -> dict[str, Any]:
        if not config_path.exists():
            logger.warning("Config file not found for workflow_optimization: %s", config_path)
            return {}
        try:
    ...
    def _discover_subtechniques(self) -> None:
        base = "sibyl.techniques.infrastructure.workflow_optimization.subtechniques"
        mapping = {
            "adaptive_retrieval": [
                (
                    f"{base}.adaptive_retrieval.default.dynamic_strategy",
                    "DynamicStrategyImplementation",
                ),
            ],
            "early_stopping": [
                (
                    f"{base}.early_stopping.default.confidence_threshold",
                    "ConfidenceThresholdImplementation",
                ),
            ],
            "parallel_execution": [
                (f"{base}.parallel_execution.default.multi_thread", "MultiThreadImplementation"),
                (f"{base}.parallel_execution.default.multi_process", "MultiProcessImplementation"),
            ],
            "query_routing": [
                (f"{base}.query_routing.default.routing_rules", "RoutingRulesImplementation"),
                (f"{base}.query_routing.default.ml_routing", "MlRoutingImplementation"),
            ],
            "fallback_strategies": [
                (f"{base}.fallback_strategies.default.cascade", "CascadeImplementation"),
                (
                    f"{base}.fallback_strategies.default.circuit_breaker",
                    "CircuitBreakerImplementation",
                ),
            ],
            "cost_optimization": [
                (f"{base}.cost_optimization.default.token_budget", "TokenBudgetImplementation"),
                (f"{base}.cost_optimization.default.api_limits", "ApiLimitsImplementation"),
            ],
        }
    ...
    def _load_and_register(self, subtechnique: str, module_path: str, class_name: str) -> None:
        try:
            ...
            logger.warning(
                "Failed to register workflow_optimization implementation %s.%s: %s",
                module_path,
                class_name,
                exc,
            )
